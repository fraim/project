...
import { CartBadge, cartActions, cartSelectors } from '@entities/cart';
...
export const Header: FC = () => {
	...
	const cartCount = useAppSelector(cartSelectors.getCartCount);

	const likeCount = useMemo(() => (
		products.filter((p) => isLiked(p.likes, user?.id)).length
	), [products]);
		...
		<header className={s.header}>
			<div className={classNames('container', s.header__wrapper)}>
				...
				{accessToken && (
					<>
						<SearchProducts />
						<nav className={s['header__icons-menu']}>
							<Link className={s['header__favorites-link']} to='/favorites'>
								<CartBadge count={likeCount} icon={HeartIcon} />
							</Link>
							<Link className={s['header__favorites-link']} to='/cart'>
								<CartBadge count={cartCount} icon={CartIcon} />
							</Link>
